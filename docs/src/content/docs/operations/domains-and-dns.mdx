...
description: Configure custom domains, DNS providers, and automatic hostnames for Vessl services.
---

Vessl uses Traefik to route traffic and manage certificates once DNS resolves to your server. It also supports automated DNS record management through major providers.
...
Vessl can create or update service-domain `A` records automatically through supported DNS providers:
- **Cloudflare**: Supports API token or auth key. When zone ID is blank, Vessl searches Cloudflare zones from the hostname.
- **Namecheap**: Requires API user, API key, and Client IP.
- **Spaceship**: Requires API key and API secret.

### What Automation Does

From a service Domains tab, Vessl can apply an `A` record for a public hostname directly to your DNS provider. The record points at the server's public IPv4 address.
