...
Vessl supports natively deploying applications defined via standard `docker-compose.yml` files.

```bash
vessld deploy --compose ./docker-compose.yml
```

When Vessl detects a `docker-compose.yml` file, it parses the file:
<Steps>
1. **Services:** Iterates over the `services` block and registers each as an individual App Service.
2. **Environment Variables:** Values in `environment` or `env_file` are imported as Vessl Secrets.
...
Docker image services skip the source build. Vessl pulls the configured image and runs it directly on the server. Use this when:
...
Vessl supports serverless function deployment for lightweight, event-driven workloads directly from the dashboard using the in-browser Monaco editor.
