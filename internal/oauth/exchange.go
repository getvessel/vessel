...
func exchangeGitHub(client *http.Client, p *Provider, code string) (string, error) {
...
func exchangeBitbucket(client *http.Client, p *Provider, code string) (string, error) {
	values := url.Values{
		"client_id": {p.ClientID}, "client_secret": {p.ClientSecret},
...
func exchangeOIDC(client *http.Client, p *Provider, code string) (string, error) {
...
func oidcEndpoints(p *Provider) (tokenURL, userURL string) {
	switch strings.ToLower(p.ProviderName) {
	case "gitlab":
		base := p.BaseURL
		if base == "" {
