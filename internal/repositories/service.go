...
func (r *AppServiceSQLiteRepository) Create(_ context.Context, svc *models.AppService) error {
	...
	_, err := r.db.Exec(
		`INSERT INTO app_services (id, project_id, environment_id, name, repository_url, image_ref, branch, root_directory, runtime_mode, build_command, start_command, dockerfile_path, build_engine, internal_port, domain, static_output, health_check_path, container_id, status, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
...
func (r *AppServiceSQLiteRepository) GetByID(ctx context.Context, id string) (*models.AppService, error) {
	...
	err := r.db.GetContext(ctx, &svc,
		`SELECT id, project_id, environment_id, name, repository_url, COALESCE(image_ref,'') AS image_ref, branch, root_directory, runtime_mode, build_command, start_command, dockerfile_path, build_engine, internal_port, domain, COALESCE(static_output,'') AS static_output, health_check_path, container_id, status, created_at, updated_at
...
func (r *AppServiceSQLiteRepository) Update(_ context.Context, svc *models.AppService) error {
	...
	_, err := r.db.Exec(
		`UPDATE app_services SET
			name = ?, repository_url = ?, image_ref = ?, branch = ?, root_directory = ?, runtime_mode = ?, build_command = ?, start_command = ?, dockerfile_path = ?, build_engine = ?, internal_port = ?, domain = ?, static_output = ?, health_check_path = ?, container_id = ?, status = ?, updated_at = ?
		WHERE id = ?`,
		svc.Name, svc.RepositoryURL, svc.ImageRef, svc.Branch, svc.RootDirectory, svc.RuntimeMode, svc.BuildCommand, svc.StartCommand, svc.DockerfilePath, svc.BuildEngine, svc.InternalPort, svc.Domain, svc.StaticOutput, svc.HealthCheckPath,
