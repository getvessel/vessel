...
func (r *AppServiceRepo) Create(_ context.Context, svc *models.AppService) error {
	...
	_, err := r.db.Exec(
		`INSERT INTO app_services (id, project_id, environment_id, name, repository_url, image_ref, branch, root_directory, runtime_mode, install_command, build_command, start_command, dockerfile_path, build_engine, internal_port, domain, static_output, health_check_path, container_id, status, replicas, cpu_limit, memory_limit, deploy_token, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		...
		svc.InternalPort, svc.Domain, svc.StaticOutput, svc.HealthCheckPath, svc.ContainerID, svc.Status, svc.Replicas, svc.CPULimit, svc.MemoryLimit, svc.DeployToken, svc.CreatedAt, svc.UpdatedAt,
	)
	if err != nil {
...
func (r *AppServiceRepo) GetByID(ctx context.Context, id string) (*models.AppService, error) {
	...
	err := r.db.GetContext(ctx, &svc,
		`SELECT id, project_id, environment_id, name, repository_url, COALESCE(image_ref,'') AS image_ref, branch, root_directory, runtime_mode, COALESCE(install_command,'') AS install_command, build_command, start_command, dockerfile_path, build_engine, internal_port, domain, COALESCE(static_output,'') AS static_output, health_check_path, container_id, status, replicas, COALESCE(cpu_limit, 0) AS cpu_limit, COALESCE(memory_limit, 0) AS memory_limit, COALESCE(deploy_token,'') AS deploy_token, created_at, updated_at
...
func (r *AppServiceRepo) Update(_ context.Context, svc *models.AppService) error {
	...
	_, err := r.db.Exec(
		...
		internal_port = ?, domain = ?, static_output = ?, health_check_path = ?, container_id = ?, status = ?, replicas = ?, cpu_limit = ?, memory_limit = ?, deploy_token = ?, updated_at = ?
		WHERE id = ?`,
		...
		svc.InternalPort, svc.Domain, svc.StaticOutput, svc.HealthCheckPath, svc.ContainerID, svc.Status, svc.Replicas, svc.CPULimit, svc.MemoryLimit, svc.DeployToken, svc.UpdatedAt, svc.ID,
	)
	if err != nil {
