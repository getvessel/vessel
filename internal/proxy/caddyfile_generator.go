...
// Generate formats a Caddyfile string containing global TLS configurations, default project routes, and custom domain proxies.
func (g *CaddyfileGenerator) Generate(projects []types.ProjectConfig, domains []types.DomainConfig) (string, error) {
...
func (g *CaddyfileGenerator) writeProjectBlock(buf *bytes.Buffer, p *types.ProjectConfig) {
	...
	if p.Domain != "" {
		hostnames = append(hostnames, strings.TrimSpace(p.Domain))
	}
	if p.Name != "" {
